<script lang="ts">
  ...
  interface Props {
    ...
    ondelete?: (rule: Rule | null) => void;
    onclone?: (rule: Rule) => void;
    onclose?: () => void;
  }

  let { open = $bindable(false), rule = null, onsave, ondelete, onclone, onclose }: Props = $props();

  let dialog: HTMLDialogElement | undefined = $state();
  let name = $state('');
  let subject = $state<Subject>('any_chapter');
  let predicates: Predicate[] = $state([]);
  let showDeleteConfirm = $state(false);

  // Contextual help text
  const SUBJECT_HELP: Partial<Record<Subject, string>> = {
  ...
  const PART2_HELP: Partial<Record<Part2, string>> = {
    text_similar:
      'Matches text that is similar to the provided value (does not require an exact match). Always case-insensitive.',
    book_title_similar:
      'Matches text that is similar to the book title (does not require an exact match). Always case-insensitive.',
    regex:
  ...
  $effect(() => {
    if (open) {
      if (rule) {
        ...
        predicates = JSON.parse(JSON.stringify(rule.predicates || [])) as Predicate[];
      } else {
        name = '';
        subject = 'any_chapter';
  ...
  let isCountSubject = $derived(subject === 'chapter_count');
  let canMultiPredicate = $derived(MULTI_PREDICATE_SUBJECTS.has(subject));
  ...
  let previewName = $derived(autoRuleName(previewRule));

  // Part2 options that need a value input
  function needsTextValue(part2: Part2) {
    return part2 === 'text' || part2 === 'text_similar' || part2 === 'regex';
  }
  function showIgnoreCase(pred: Predicate) {
    return pred.kind === 'text' && (pred.part2 === 'text' || pred.part2 === 'regex');
  }

  function isMatchesOp(op: TextOp | CountOp) {
  ...
  <div class="dialog-container">
    ...
    <div class="dialog-body">
      ...
      <div class="field">
        ...
        <input id="rule-name" type="text" bind:value={name} placeholder={previewName} />
      </div>

      <!-- Subject -->
      <div class="field">
        <label for="rule-subject">Target</label>
        <select id="rule-subject" bind:value={subject} onchange={handleSubjectChange}>
      ...
      <div class="field">
        ...
        {#if canMultiPredicate}
          <button class="add-pred-btn" onclick={addPredicate} type="button">
    ...
    <div class="dialog-footer">
      ...
      {/if}
      {#if rule}
        <button class="btn btn-secondary" onclick={handleClone}>Clone</button>
      {/if}
