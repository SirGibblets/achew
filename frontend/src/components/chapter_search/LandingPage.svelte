<script lang="ts">
  ...
  let libraries = $derived(($chapterSearch.libraries as Library[] | undefined) || []);
  let rootRuleset = $derived($chapterSearch.rootRuleset);
  let selectedLibrary = $derived(libraries.find((lib) => lib.id === selectedLibraryId) || null);
  let canSearch = $derived(!!(selectedLibrary && rootRuleset && hasEnabledRules(rootRuleset)));
  ...
  async function handleSearch() {
    if (!canSearch || !selectedLibrary) return;
    chapterSearch.startSearch(selectedLibrary.id, selectedLibrary.name);
  }

  async function handleRulesetChange(newRuleset: RuleSet) {
...
<div class="landing-page">
  ...
  <div class="header-row">
    <div class="library-row">
      <select class="library-select" bind:value={selectedLibraryId} disabled={libraries.length === 0}>
