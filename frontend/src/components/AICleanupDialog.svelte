<script lang="ts">
  ...
  async function loadAvailableProviders() {
    try {
      ...
      const configuredProviders = availableProviders.filter((p) => p.is_enabled && p.is_configured);
  ...
  async function loadAvailableModels(providerId: string) {
    ...
    try {
      ...
      if (aiOptions.model_id === 'gpt-4o' && providerId === 'ollama') {
        ...
        const commonModels = ['llama3.2:latest', 'llama3.1:latest', 'gemma2:latest'];
        const foundModel = availableModels.find((m) => commonModels.some((cm) => m.id.includes(cm.split(':')[0])));
        if (foundModel) {
  ...
  async function handleProviderChange(newProviderId: string) {
    ...
    >
      <div class="ai-modal-header">
        <div class="ai-modal-header-content">
          ...
          <h3>
            AI Chapter Cleanup &nbsp;<DocLink path="/editor/ai-cleanup/" featureName="AI Cleanup" size="14" />
          </h3>
        </div>

        <div class="features-toggle" onclick={() => (featuresCollapsed = !featuresCollapsed)}>
          ...
          {#if !featuresCollapsed}
            <div class="ai-features-content" transition:slide={{ duration: 300 }}>
              <p class="ai-features-header">
                This process uses AI to intelligently clean up the selected chapter titles. It will attempt to:
              </p>
          ...
          <div class="ai-options-column">
            ...
            {#if availableProviders.filter((p) => p.is_enabled && p.is_configured).length === 0}
              <div class="no-providers-message">
                ...
                <p>
                  No LLM providers are configured. Please set up an AI provider in the settings to use this feature.
                </p>
              </div>
            {:else}
            ...
            <div class="checkbox-group">
              <label class="checkbox-label">
                <input type="checkbox" bind:checked={aiOptions.inferOpeningCredits} />
      ...
      <div class="ai-modal-footer">
        <div class="ai-modal-actions">
          <button class="btn btn-cancel btn-ai-cancel" onclick={cancelAICleanup}> Cancel </button>
          <button
            class="btn ai-confirm-btn"
            onclick={confirmAICleanup}
            disabled={loadingModels || availableProviders.filter((p) => p.is_enabled && p.is_configured).length === 0}
          >
...
<style>
  ...
  .ai-confirm-btn:hover {
    background: linear-gradient(
      135deg,
      var(--ai-gradient-start-hover) 0%,
      var(--ai-gradient-end-hover) 100%
    ) !important;
  }
