<script lang="ts">
  import { session, transcriptionStatuses } from '../stores/session';
  ...
  interface Props {
    onasrsetupcomplete?: () => void;
  }

  let { onasrsetupcomplete }: Props = $props();
  ...
  async function handleSave() {
    loading = true;
    try {
      const response = await fetch('/api/complete-asr-setup', { method: 'POST' });
  ...
  let hasActiveTranscriptions = $derived(Object.keys($transcriptionStatuses).length > 0);
</script>
...
<div class="asr-setup">
  ...
  <div class="actions">
    <button class="btn btn-cancel" onclick={handleCancel} disabled={loading}> Cancel </button>
    <button class="btn btn-verify" onclick={handleSave} disabled={loading}>
      {#if loading}
