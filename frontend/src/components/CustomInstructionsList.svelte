<script lang="ts">
  ...
  async function loadInstructions() {
    try {
      loading = true;
      const response = (await api.batch.getCustomInstructions()) as InstructionsResponse;
  ...
  async function addInstruction() {
    ...
    setTimeout(() => {
      const newInput = document.querySelector(`input[data-id="${newInstruction.id}"]`) as HTMLInputElement | null;
      if (newInput) {
        newInput.focus();
      }
    }, 50);
  ...
  async function toggleChecked(id: string) {
    instructions = instructions.map((inst) => (inst.id === id ? { ...inst, checked: !inst.checked } : inst));
    await saveInstructionsImmediate();
  }

  function updateText(id: string, text: string) {
    instructions = instructions.map((inst) => (inst.id === id ? { ...inst, text } : inst));
  ...
  async function handleDndFinalize(e: CustomEvent<DndEvent<Instruction>>) {
  ...
  function handleKeydown(event: KeyboardEvent, _id: string) {
    if (event.key === 'Enter') {
...
<div class="custom-instructions">
  ...
  {:else}
      ...
      {#each instructions as instruction (instruction.id)}
        <div class="instruction-wrapper" data-id={instruction.id}>
          <label class="checkbox-container">
            <input type="checkbox" checked={instruction.checked} onchange={() => toggleChecked(instruction.id)} />
          ...
          <div class="instruction-item">
            <div class="text-input-container">
              <input
                ...
                oninput={(e) => updateText(instruction.id, (e.target as HTMLInputElement).value)}
    ...
    <button class="add-btn" onclick={addInstruction} type="button">
      <Plus size="16" />
      Add Instruction
    </button>
  {/if}
