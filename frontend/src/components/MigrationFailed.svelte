<script lang="ts">
  ...
  interface Props {
    onmigrationreset?: () => void;
  }

  let { onmigrationreset }: Props = $props();
  ...
  async function handleReset() {
    try {
      ...
      const response = await fetch('/api/config/migration/reset', { method: 'POST' });
...
<div class="migration-container">
  ...
  <div class="setup-form">
      ...
      <div class="provider-header">
        <div class="provider-info">
          ...
          <p class="provider-desc">
            Your settings were saved in a format that is incompatible with this version. This can happen when the
            underlying Python version changes during an update. Unfortunately, the old settings cannot be recovered
            automatically.
          </p>
          <h2 style="margin-top: 1.25rem;">
            To continue, you must either reset your settings or downgrade Achew to a previous version.
          </h2>
          <p class="provider-desc">
            After resetting, you will need to re-enter any of the following that were previously configured:
          </p>
    ...
    <div class="form-actions">
      <button class="btn btn-verify" disabled={loading} onclick={handleReset}>
        {#if loading}
