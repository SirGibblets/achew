<script lang="ts">
  ...
  interface Props {
    onabsconfigured?: () => void;
  }

  let { onabsconfigured }: Props = $props();
  ...
  let validationStatus = $state<ValidationStatus>({ valid: false, message: 'Not configured' });
  let configLoaded = $state(false);
  let isInitialSetup = $state(false);
  ...
  async function handleVerifyAndSave() {
    try {
      ...
      const apiKeyToSend = absApiKey.startsWith('••••') ? absApiKey : absApiKey.trim();
      ...
      const data = (await response.json()) as ABSSetupResponse;
      ...
      if (data.success) {
        ...
        if (data.errors?.abs) {
          ...
          validationStatus = { valid: false, message: data.message ?? 'Validation failed' };
        }
      }
    } catch (error) {
  ...
  async function handleCancel() {
    try {
      ...
      const response = await fetch('/api/config/abs/setup', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'cancel', url: '', api_key: '' }),
      });
  ...
  let statusClass = $derived(
    validationStatus.valid ? 'success' : validationStatus.message === 'Not Configured' ? 'subtle' : 'error',
  );

  let statusText = $derived(validationStatus.message || 'Not Configured');

  let showCancelButton = $derived(configLoaded && !isInitialSetup);
  let cancelDisabled = $derived(loading);
...
<div class="abs-setup-container">
  <div class="setup-header">
    ...
    <h1>
      Audiobookshelf Setup <DocLink
        path="/installation/first-run/#audiobookshelf-setup"
        featureName="Audiobookshelf Setup"
        size="16"
      />
    </h1>
  ...
  <div class="setup-form">
    <div class="provider-section">
      <div class="provider-header">
        <div class="provider-info">
          ...
          <p class="provider-desc">Enter your Audiobookshelf URL and API key.</p>
          <p class="api-key-help">
            <a
              class="api-key-help"
              href="https://www.audiobookshelf.org/guides/api-keys/#creating-api-keys"
              target="_blank"
      ...
      <div class="provider-form">
        <div class="form-group">
          <p class="field-hint">Server URL</p>
          <input
            id="abs-url"
            type="url"
            autocomplete="off"
    ...
    <div class="form-actions">
      {#if showCancelButton}
        <button class="btn btn-cancel" disabled={cancelDisabled} onclick={handleCancel}> Cancel </button>
      {/if}
      <button class="btn btn-verify" disabled={loading} onclick={handleVerifyAndSave}>
        {#if loading}
