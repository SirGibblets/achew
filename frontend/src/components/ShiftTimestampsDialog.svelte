<script lang="ts">
  ...
  let selectedChapters = $derived($chapters.filter((ch) => !ch.deleted && ch.selected));
  ...
  let canApply = $derived(affectedChapters.length > 0 && (offset !== 0 || (driftEnabled && drift !== 0)));

  function computeNewTimestamps(): Array<{ chapter_id: string; new_timestamp: number }> {
    const result: Array<{ chapter_id: string; new_timestamp: number }> = [];
  ...
  let changedCount = $derived(previewData.filter((item) => item.oldTimestamp !== item.newTimestamp).length);
  ...
  async function togglePlayPreviewItem(id: string, newTimestamp: number) {
  ...
  function adjustDrift(delta: number) {
    stopPlayback();
  ...
  function handleDriftKeydown(e: KeyboardEvent) {
    if (e.key === 'ArrowUp') {
      e.preventDefault();
      adjustDrift(1);
    } else if (e.key === 'ArrowDown') {
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h3>
            Shift Timestamps &nbsp;<DocLink
              path="/editor/shift-timestamps/"
              featureName="Shifting Timestamps"
              size="14"
            />
          </h3>
              ...
              <div class="preview-list">
                {#each previewData as item}
                  <div class="preview-row">
                    ...
                    <span class="preview-title" class:no-title={!item.title} title={item.title ?? 'No Title'}
                    ...
                    <span class="preview-timestamps">
                      ...
                      <span class="ts-new" class:ts-changed={item.oldTimestamp !== item.newTimestamp}
                        >{formatTimestamp(item.newTimestamp)}</span
                      >
                    </span>
                  </div>
                {/each}
